...
function setText(text, param2) {
    ...
    if (param2 != null) roundStart.param2 = param2; //	Set position, default center
}

function checkEnemyTouchBullet(enemyObj) {
    //	Null for check tree
    for (var i in bulletList) {
        if (bulletList[i].xCanvas > canvasWidth) {
            //	Far then Width Sceen
            delete bulletList[i];
            continue;
        }
        for (var treeNum in treeList) {
            //	Check touch Tree
            if (
                bulletList[i].treeID == null &&
                checkTouch(bulletList[i], treeList[treeNum])
            ) {
        ...
        if (
            ...
            bulletList[i] != null &&
            bulletList[i].treeID == null &&
            checkTouch(bulletList[i], enemyObj) &&
            enemyObj.display == true
        ) {
            //	Check touch enemy
            enemyObj.hp--;
            delete bulletList[i];
...
function arrowUpdate() {
    for (var i in arrowEnemyList) {
        if (arrowEnemyList[i].display) arrowEnemyList[i].draw();
        else delete arrowEnemyList[i];
    }
}

function fireballUpdate() {
    for (var i in fireballList) {
        if (fireballList[i].display) fireballList[i].draw();
        else delete fireballList[i];
    }
}

function fireBossUpdate() {
    for (var i in fireList) {
        if (fireList[i].display) fireList[i].draw();
        else delete fireList[i];
    }
}
    ...
    for (var i in bulletList) {
        if (bulletList[i].display) {
            if (bulletList[i].treeID != null) {
                ...
                bulletList[i].xCanvas +=
                    bulletList[i].speed * Math.cos(bulletList[i].degree);
                bulletList[i].yCanvas +=
                    bulletList[i].speed * Math.sin(bulletList[i].degree);
            }
            ctx.save();
            ctx.translate(
                bulletList[i].xCanvas + car.width / 2,
                bulletList[i].yCanvas + car.height / 2
            );
...
function enemyUpdate() {
    if (level == 1 && secondAttack == false) {
        if (enemySpam <= enemyTotal - 1 && deplay != true) {
            ...
            if (enemySpam == enemyTotal / 2) {
                ...
                setTimeout(function () {
                    deplay = false;
                }, timeSpam); // If not deplay a function
        }

        if (enemySpam >= enemyTotal && checkCleanEnemy()) {
            //Ready for lv2
            readyLv2 = true;
            secondAttack = true;
            recreateHeart();
    ...
    if (level == 2 && secondAttack == false) {
        //	Level 2
        if (
            bossLevel2 == false &&
            enemySpam >= enemyTotal &&
            checkCleanEnemy()
        ) {
            recreateHeart();
            createEnemy(
    ...
    if (level == 3 && secondAttack == false) {
        ...
        if (enemySpam >= enemyTotal && checkCleanEnemy() && bossLevel3) {
            ...
            setText('WINNER!!!!', '40%');
            setTimeout(() => {
                location.href = '/dyl-editor/game/win/index.html?type=46';
            }, 1000);
        }
    }
...
function displayEnemy() {
    for (var key in enemyList) {
        ...
        } else delete enemyList[key]; //	Release if don't exist
    }
}

function checkCleanEnemy(agm) {
    //	null for default check
    var checking = false;
    for (var key in enemyList) {
        if (enemyList[key].display) {
            //	Enemy still have
    ...
    if (checking == false && level == 1 && agm == 'loop') {
        ...
        recreateHeart();
        return;
    } else if (checking == false && agm == null) return true;
}
...
function TreeAndHeart() {
    ...
    if (currentHeart <= totalHeart - 1 && displayHeart) {
        ...
        var xCreate = Math.floor(
            Math.random() * car.xCanvas + 100 + canvasWidth
        );
        heartList[currentHeart] = new Object(
            xCreate,
            yCreate,
            7,
            100,
    ...
    for (var i in heartList) {
        ...
        if (heartList[i].display) {
            ...
            heartList[i].draw();
            if (
                car.xCanvas - 2 * Math.cos(degreeChange) >
                    heartList[i].xCanvas &&
                car.xCanvas - 2 * Math.cos(degreeChange) <
                    heartList[i].xCanvas + heartList[i].width / 2 &&
                car.yCanvas - 39 < heartList[i].yCanvas + heartList[i].height &&
                car.yCanvas + 39 > heartList[i].yCanvas
            ) {
                if (50 - car.hp < 3) car.hp += 50 - car.hp;
                else car.hp += 3;
                delete heartList[i];
            }
        }
        ...
        if (starList[i].display) {
            ...
            if (
                car.xCanvas - 2 * Math.cos(degreeChange) >
                    starList[i].xCanvas &&
                car.xCanvas - 2 * Math.cos(degreeChange) <
                    starList[i].xCanvas + starList[i].width / 2 &&
                car.yCanvas - 39 < starList[i].yCanvas + starList[i].height &&
                car.yCanvas + 39 > starList[i].yCanvas
            ) {
                getCar().armor++;
                delete starList[i];
            }
...
function setVariable(levelC) {
    if (levelC == 2) {
        ...
        secondAttack = false;
        secondAttackLocation = [
            100, 200, 10, 100, 200, 120, 140, 160, 180, 230,
        ]; //EDIT
    ...
    if (levelC == 3) {
        ...
        secondAttack = false;
        secondAttackLocation = [
            100, 200, 10, 100, 200, 120, 140, 160, 180, 230, 100, 200, 10, 100,
            200,
        ]; //EDIT
        currentSecondAttack = 0;
...
function keyDownHandler(e) {
    if (e.key == 'Right' || e.key == 'ArrowRight') car.rightkey = true;
    else if (e.key == 'Left' || e.key == 'ArrowLeft') car.leftkey = true;

    if (e.key == 'ArrowUp') car.upkey = true;
    else if (e.key == 'ArrowDown') car.downkey = true;

    if (e.keyCode == 32) car.createBullet = true;
}

// check release key
function keyUpHandler(e) {
    if (e.key == 'Right' || e.key == 'ArrowRight') car.rightkey = false;
    else if (e.key == 'Left' || e.key == 'ArrowLeft') car.leftkey = false;

    if (e.key == 'ArrowUp') car.upkey = false;
    else if (e.key == 'ArrowDown') car.downkey = false;

    if (e.keyCode == 32) car.createBullet = false;
}
